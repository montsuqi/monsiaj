...
有効にすると画面をディスプレイ全体に拡大表示します。画面の縦横比はディスプレイに合わせて変化します。そのため解像度 1280 * 800 等のワイドディスプレイでは全体的に横長に表示します。
また画面の拡大比率を任意設定することは出来ません。常に最大表示します。

なおフォントのサイズは自動では変更しません。解像度が1680 * 1024等の大画面ディスプレイの場合フォントのサイズを大きくした方が良いでしょう。
...
* J2SE 1.4.2の場合でJava Web Startが利用できない
* Mac OS Xルック＆フィールで使用している場合、一部のラベルが完全に表示されない
** (R03)レセプト作成−作成指示画面の自賠責レセプトの選択欄、「従来様式」が「従来様...」となる
